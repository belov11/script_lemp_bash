...
if ! sudo test -f $nginx_path_cert/$nginx_public_key && ! sudo test -f $nginx_path_cert/$nginx_private_key; then
    sudo openssl req -x509 -nodes -days 365 -newkey rsa:4096 -keyout $nginx_path_cert/$nginx_private_key -out $nginx_path_cert/$nginx_public_key -subj "/C=/ST=/L=/O=/OU=/CN="
fi
